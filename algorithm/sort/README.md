...
#####从后向前，在已排序的数组中插入key的元素。
    1.发明者Hollerith创立的电脑制表记录公司（CTR）在1924年更名为IBM。
    2.通常采用in-place排序（即只需用到O(1)的额外空间的排序）。
    3.STL的sort算法和stdlib的qsort算法中，都将插入排序作为快速排序的补充，
      用于少量元素的排序（通常为8个或以下）。
...
#####通过两两比较交换，把最小的元素“浮”到数据的顶端。
    1.在最坏的情况，冒泡排序需要O(n^2)次交换，而插入排序只要最多O(n)交换。
    2.在内部循环第一次执行时，用一个旗标来表示有无需要交换的可能，可能把最好的复杂度降低到O(n);
    3.若在每次走访数列时，把走访顺序和比较大小反过来，也可以稍微地改进效率。
      有时候称为`往返排序`，因为算法会从数列的一端到另一端之间穿梭往返。
